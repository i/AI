...
/**
 * Maze Assignment: MazeReader.java
 *
 ...
 * @version 1.0
 */
public class MazeReader extends DefaultHandler {
    ...
    /** XML element for horizontal walls */
    static final String BEAM_NAME = "beam";
    
    /** XML element for vertical walls */
    static final String POLE_NAME = "pole";
    
    /** XML tag for x coordinate */
    static final String X_PARAM = "x";
    
    /** XML tag for y coordinate */
    ...
    throws SAXException
    {
        ...
        if (World.XML_NAME.equals(name)) {
            ...
            int cells = getIntParam(atts, World.CELL_PARAM, World.DEFAULT_CELLS, locator);
            int delay = getIntParam(atts, World.WAIT_INTERVAL, World.DEFAULT_WAIT, locator);
            int rep = getIntParam(atts, World.REPLAY_INTERVAL, World.DEFAULT_WAIT, locator);
            ...
            world = new World(width, height, cells, logfile, runnable, delay, rep, debug);
        ...
        if (BEAM_NAME.equals(name)) {
        	int x = getIntParam(atts, X_PARAM, 0, locator);
        	int y = getIntParam(atts, Y_PARAM, 0, locator);
        	world.addBeam(x, y);
        	return;
        }
 
        if (POLE_NAME.equals(name)) {
        	int x = getIntParam(atts, X_PARAM, 0, locator);
        	int y = getIntParam(atts, Y_PARAM, 0, locator);
        	world.addPole(x, y);
        	return;
        ...
        if (Follower.XML_NAME.equals(name)) {
            if (inDefaults) {
        ...
        } else if (Stepper.XML_NAME.equals(name)) {
            if (inDefaults) {
                Stepper.defaultDynamicAgentAttributes.update(atts, locator);
                Stepper.defaultFixedAgentAttributes.update(atts, locator);
            } else {
                Stepper f = new Stepper(world, id, atts, locator);
                world.addAgent(f);
            }
        } else if (TrialAndError.XML_NAME.equals(name)) {
            if (inDefaults) {
                TrialAndError.defaultDynamicAgentAttributes.update(atts, locator);
                TrialAndError.defaultFixedAgentAttributes.update(atts, locator);
            } else {
                TrialAndError f = new TrialAndError(world, id, atts, locator);
                world.addAgent(f);
            }
        } else {
